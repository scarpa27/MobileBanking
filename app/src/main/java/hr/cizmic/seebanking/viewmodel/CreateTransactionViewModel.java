...
public class CreateTransactionViewModel extends ViewModel {
    ...
    private final SingleLiveEvent<Boolean> created = new SingleLiveEvent<>();

    public CreateTransactionViewModel(BankingRepository repo, AppExecutors executors) {
        this.repo = repo;
    ...
    public LiveData<Boolean> created() { return created; }
    ...
    public void create(Long accountId,
        ...
        if (accountId == null || accountId <= 0) {
            message.setValue("Pick an account");
            return;
        }

        String mobileNorm = ValidationUtils.normalizePhone(recipientMobile);
        String ibanNorm = ValidationUtils.normalizeIban(recipientIban);
        ...
        String nameNorm = ValidationUtils.trimToNull(counterpartyName);
        ...
        repo.createTransaction(accountId, amount, nameNorm, mobileNorm, ibanNorm, noteNorm,
                r -> executors.mainThread().execute(() -> {
